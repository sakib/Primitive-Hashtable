public class Hash<T> {
    ...
    public T delete(String key) {
        try {
            if (this.load() < 0.5) {
                // Dynamically reduce the size of the hash map
                String[][] old_key_list = this.key_list;
                T[][] old_value_list = this.value_list;
                int[] old_cap_list = this.cap_list;
                this.TABLE_SIZE /= 2;
                if (this.TABLE_SIZE == 0) this.TABLE_SIZE = 1;
                this.key_list = new String[this.TABLE_SIZE][];
                this.value_list = (T[][]) (new Object[this.TABLE_SIZE][]);
                this.cap_list = new int[this.TABLE_SIZE];
                this.capacity = 0;
                ...
                for (int i = 0; i < this.TABLE_SIZE; i++) {
                    // New #s of columns should be on average twice that of old.
                    // The # of rows halved so the average # of cols should double.
                    this.key_list[i] = new String[1+old_cap_list[i*2]*2];
                    this.value_list[i] = (T[]) (new Object[1+old_cap_list[i*2]*2]);
                    this.cap_list[i] = 0;
                }
                ...
                for (int i = 0; i < this.TABLE_SIZE*2; i++) {
                    for (int j = 0; j < old_cap_list[i]; j++) {
                        ...
                        this.key_list[hash][this.cap_list[hash]] = k;
                        this.value_list[hash][this.cap_list[hash]] = val;
                        this.cap_list[hash]++;
                        this.capacity++;
                    }
                }
